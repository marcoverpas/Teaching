#A simple neoclassical model (flexible prices and wages)
...
l0 = 45     #Labour demand when wages = 0
...
#Nominal wage rate
w=matrix(data=wr/p,nrow=nScenarios,ncol=nPeriods)
...
for (t in 2:nPeriods){
  ...
  p[j,t] = Y[j,t]/X[j,t]          #Price level (market clearing)
  
  X[j,t] = Ld[j,t]*prod           #Real output (defined by production function)
  ...
  wr[j,i] = l0/(l2 + l1)          #Real wage rate (such that: Ld = Ls)        
  
  w[j,t] = wr[j,t]*p[j,t]         #Nominal wage rate (fully flexible)
